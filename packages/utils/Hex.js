...
Hex.toMap = (arr, key, val, init = Object.create(null)) =>
...
Hex.validateConfirmPass = pass => (rule, value, callback) => {
  if (value === '') {
...
Hex.validataZhLength = (min, max) => (rule, value, cb) => {
  value = value || '';
  const doubleCharLength = value.replace(/\p{Unified_Ideograph}/gu, 'aa').length;
...
Hex.validataLink = (rule, value, cb) => {
  ...
  // 不可以包含汉字（汉字应提前被转码）
  if (/\p{Unified_Ideograph}/gu.test(value)) {
